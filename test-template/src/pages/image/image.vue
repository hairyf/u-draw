<template>
  <div class="index">
    ...
    <canvas
      id="canvas"
      type="2d"
      style="width: 450px; height: 1100px; left: 75rpx; top: 15rpx"
    />
  </div>
</template>
<script lang="ts">
...
export default Vue.extend({
  ...
  async onReady() {
    ...
    dp.draw(async (ctx) => {
      ...
      dp.draw(async (ctx) => {
        const url = 'https://s1.ax1x.com/2020/10/28/B3G861.jpg';
        await ctx.drawImage(url, 0, 0, 600, 1100);
      });
    });
    // 绘制图片(圆角)
    // dp.draw(async (ctx) => {
